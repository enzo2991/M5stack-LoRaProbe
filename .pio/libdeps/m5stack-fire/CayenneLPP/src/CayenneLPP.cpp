...
bool CayenneLPP::isType(uint8_t type) {
  ...
  switch (type) {
    ...
    case LPP_PRESENCE:
#endif
#ifndef CAYENNE_DISABLE_RSSI
    case LPP_RSSI:
#endif
#ifndef CAYENNE_DISABLE_SAT
    case LPP_SAT:
#endif
#ifndef CAYENNE_DISABLE_SNR
    case LPP_SNR:
#endif
#ifndef CAYENNE_DISABLE_HDOP
    case LPP_HDOP:
#endif
...
#ifndef CAYENNE_DISABLE_RSSI
uint8_t CayenneLPP::addRssi(uint8_t channel, uint32_t value) {
  return addField(LPP_RSSI, channel, value);
}
#endif

#ifndef CAYENNE_DISABLE_SNR
uint8_t CayenneLPP::addSnr(uint8_t channel, uint32_t value) {
  return addField(LPP_SNR, channel, value);
}
#endif

#ifndef CAYENNE_DISABLE_HDOP
uint8_t CayenneLPP::addHdop(uint8_t channel, uint32_t value) {
  return addField(LPP_HDOP, channel, value);
}
#endif

#ifndef CAYENNE_DISABLE_SAT
uint8_t CayenneLPP::addSat(uint8_t channel, uint32_t value) {
  return addField(LPP_SAT, channel, value);
}
#endif
...
uint8_t CayenneLPP::decode(uint8_t *buffer, uint8_t len, std::map<uint8_t, CayenneLPPMessage> &messageMap) {
  ...
  while ((index + 2) < len) {
    ...
    case LPP_PRESENCE:
      ...
      break;
    case LPP_RSSI:
      messageMap[channel].rssi = getValue(&buffer[index], size, multiplier, is_signed);
      break;
    case LPP_SNR:
      messageMap[channel].snr = getValue(&buffer[index], size, multiplier, is_signed);
      break;
    case LPP_HDOP:
      messageMap[channel].hdop = getValue(&buffer[index], size, multiplier, is_signed);
      break;
    case LPP_SAT:
      messageMap[channel].sat = getValue(&buffer[index], size, multiplier, is_signed);
      break;
    case LPP_TEMPERATURE:
